...
def data_cost(pixel, mainColor, mainColors):
    ...
    for color in mainColors:
        sum += pow(
            pow(pixel.color[0] - color[0], 2)
            + pow(pixel.color[1] - color[1], 2)
            + pow(pixel.color[2] - color[2], 2),
            0.5,
        )
    colorSum = pow(
        pow(pixel.color[0] - mainColor[0], 2)
        + pow(pixel.color[1] - mainColor[1], 2)
        + pow(pixel.color[2] - mainColor[2], 2),
        0.5,
    )
    try:
...
def find_clusters(pixels, allColors):
    ...
    for color in allColors:
        ...
        if len(clusteredPixel) > 0:
            ...
            clusterMean.append(
                [clustMean[0] / len(clusteredPixel), clustMean[1] / len(clusteredPixel)]
            )
    ...
    for i in range(len(clusters)):
        if set(pixels[clusters[i][0][0]][clusters[i][0][1]].label) != set(mainColor):
            ...
            for pos in clusters[i]:
                mainColorCost += (
                    pixels[pos[0]][pos[1]].cost
                    - pixels[pos[0]][pos[1]].colorCost[str(mainColor)]
                )
                mainColorCost += smooth_cost_expansion(
                    pixels, [pos[0], pos[1]], mainColor
                )
...
def get_colors(img):
    ...
    for bin in range(0, binNum * binNum * binNum):
        if binsAmount[bin] / binTotal > 0.05:
            bins.append(
                [
                    int(binsSum[bin][0] / binsAmount[bin]),
                    int(binsSum[bin][1] / binsAmount[bin]),
                    int(binsSum[bin][2] / binsAmount[bin]),
                ]
            )
    return bins
...
def auto_segment(img):
    ...
    for mainColor in mainColors:
        bPixel = [
            [None for y in range(iar.shape[1] - 1)] for x in range(iar.shape[0] - 1)
        ]
        for z in range(0, iar.shape[0] - 1):
            for j in range(0, iar.shape[1] - 1):
    ...
    while redo:
        ...
        for mainColor in mainColors:
            change = fast_alpha_expansion(
                pixels, clusters, mainColor, mainColors, clusterChanges
            )
            if change[0]:
                redo = True
        ...
        for i in range(len(clusters)):
            ...
            if col != None and oldCol not in usedColors:
                ...
                for pos in clusters[index]:
                    ...
                    pixels[pos[0]][pos[1]].cost = pixels[pos[0]][pos[1]].colorCost[
                        str(col)
                    ]
    ...
    for i in range(len(sumVariance)):
        if sums[i][1] != 0:
            variance.append(
                [
                    pow(sumVariance[i][0] / sums[i][1], 0.5),
                    pow(sumVariance[i][1] / sums[i][1], 0.5),
            ...
            if sumSkew[i][0] < 0 or sumSkew[i][1] < 0 or sumSkew[i][2] < 0:
                ...
                skew.append(
                    [
                        pow(sumSkew[i][0] / sums[i][1], 1 / 3),
                        pow(sumSkew[i][1] / sums[i][1], 1 / 3),
